...
`,$=async()=>{let e=document.getElementById(`markdown-content`);if(e)try{e.innerHTML=await Q.parse(je),console.log(`Markdown successfully rendered.`)}catch(t){console.error(`Failed to render Markdown:`,t),e.innerHTML=`<div class="error">Failed to load content.</div>`}document.querySelectorAll(`.nav-links a`).forEach(e=>{e.addEventListener(`click`,function(e){let t=this.getAttribute(`href`);if(t.startsWith(`#`)){e.preventDefault();let n=t.slice(1),r=document.getElementById(n);r&&(window.scrollTo({top:r.offsetTop-120,behavior:`smooth`}),document.querySelectorAll(`.nav-links a`).forEach(e=>e.classList.remove(`active`)),this.classList.add(`active`))}})})};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,$):$();